...
@section('content')
    <div class="title-content-info">Edit Sub Category II</div>
    <div class="row col-md-9 well">
        <form action="{{ url('update-sub-ii/'.$sub_ii->id) }}" method="POST">
            @csrf
            <input id="cat_id" type="hidden" name="cat_id" value="{{ request()->get('id') }}">
            <input id="sub_i_id" type="hidden" name="sub_i_id" value="{{ request()->get('sub') }}">
            <div class="form-group row">
                <label for="name" class="col-md-3 col-form-label text-md-right">{{ __('Name') }}</label>

                <div class="col-md-6">
                    <input id="name" type="text" class="form-control @error('name') is-invalid @enderror" name="name" value="{{ $sub_ii->name }}">
                </div>
            </div>
            <div class="form-group row">
                ...
                <div class="col-md-6">
                    <input id="text_color" type="color" class="form-control text_color @error('text_color') is-invalid @enderror" name="text_color" value="{{ $sub_ii->text_color }}">
                </div>
            </div>
            <div class="form-group row">
                ...
                <div class="col-md-6">
                    <input id="background_color" type="color" class="form-control background_color @error('background_color') is-invalid @enderror" name="background_color" value="{{ $sub_ii->background_color }}">
                </div>
            </div>
            <div class="form-group row">
                ...
                <div class="col-md-6">
                    <input id="icon" type="text" class="form-control icon @error('icon') is-invalid @enderror" name="icon" value="{{ $sub_ii->icon }}">
                </div>
            </div>
            <div class="form-group row">
                ...
                <div class="col-md-6">
                    <textarea name="remark" id="remark" class="form-control">{{ $sub_ii->remark }}</textarea>
                </div>
            </div>
